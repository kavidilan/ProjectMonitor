...
/* ─────────────────────────────────────────────────────────────
  ...
  Sub-cols per month: PTC | PAC | FTC | FAC | Progress (PAC/PTC×100)
  ───────────────────────────────────────────────────────────── */
...
const BASE_COLS = 13;
// 5 sub-cols per month: PTC | PAC | FTC | FAC | Progress
...
export default function DataEntryTable({
  ...
  hadd,
  handleSave,
  reloadProjects, // Grab the newly added prop
  setModalProject,
  hpc,
  hmc,
  hdel,
        ...
        <div className="det-toolbar">
          ...
          <div className="det-toolbar-right">
              ...
              onChange={async (e) => {
                ...
                try {
                  ...
                  if (response.ok) {
                    ...
                    // Call the newly passed prop to fetch the fresh DB data
                    if (typeof reloadProjects === 'function') reloadProjects(); 
                  } else {
                    alert('Failed to sync Excel data.');
                  }
                } catch (error) {
            ...
            <label 
              ...
              style={{ background: 'linear-gradient(135deg, #3b82f6, #2563eb)', cursor: 'pointer', marginRight: '10px' }}
            >
        ...
        <div className="det-scroll">
          <table className="det-table">
            <thead>
              ...
              {/* ══ ROW 3 – PTC/PAC/FTC/FAC/Progress per month ══ */}
              <tr>
                {MONTHS.map((m) => (
                  <React.Fragment key={m}>
                    <th className="det-th det-th-sub det-th-pt" title="Physical Target Cumulative">PTC</th>
                    <th className="det-th det-th-sub det-th-pp" title="Physical Achieved Cumulative">PAC</th>
                    <th className="det-th det-th-sub det-th-ft" title="Financial Target Cumulative">FTC</th>
                    <th className="det-th det-th-sub det-th-fp" title="Financial Achieved Cumulative">FAC</th>
                    <th className="det-th det-th-ip" title="Progress = PAC / PTC * 100">Progress</th>
                  </React.Fragment>
                ))}
              </tr>
            </thead>
              ...
              {filteredProjects.map((p, idx) => {
                ...
                return (
                  <tr key={key}>
                    ...
                    {/* Monthly PTC/PAC/FTC/FAC/Progress × 12 */}
                    {MONTHS.map((m) => {
                        ...
                        <React.Fragment key={m}>
                          <td className="det-td det-td-month" onClick={(e) => e.stopPropagation()}>
                            <input
                              ...
                              title={`${m} – Physical Target Cumulative`}
                            />
                          </td>
                          <td className="det-td det-td-month" onClick={(e) => e.stopPropagation()}>
                            <input
                              ...
                              title={`${m} – Physical Achieved Cumulative`}
                            />
                          </td>
                          <td className="det-td det-td-month" onClick={(e) => e.stopPropagation()}>
                            <input
                              ...
                              title={`${m} – Financial Target Cumulative`}
                            />
                          </td>
                          <td className="det-td det-td-month" onClick={(e) => e.stopPropagation()}>
                            <input
